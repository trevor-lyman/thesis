...
write.csv(prop, "Outputs/model_prop_table_v2.csv")
...
new <- model_outputs %>%
  mutate(min_to_flux = total_Cmin/total_consumption) %>%
  group_by(tx) %>%
  summarize(mean = mean(min_to_flux),
            se = sd(min_to_flux)/sqrt(length(min_to_flux)))
