...
Five experimental mesocosms were destructively sampled following a three-week recovery period but prior to deployment of the experimental treatments (T~0~). The following conditions were recorded for these five control mesocosms and were assumed to be representative of field values. The mean heterotrophic respiration (&plusmn; SE) prior to implementing experimental soil moisture and temperature conditions at T~0~ was `r mean_cum_resp_T0` &plusmn; `r se_cum_resp_T0` ln(g of C per m^2^ per year), which equates to ~`r comma(round(mean(exp(resp_T0$cum_resp)), digits = get_round_digit(signif(sd(exp(resp_T0$cum_resp))/sqrt(length(resp_T0$cum_resp)), digits = 1))))` &plusmn; `r signif(sd(exp(resp_T0$cum_resp))/sqrt(length(resp_T0$cum_resp)), digits = 1)` g of C per m^2^ per year. The mean soil moisture (&plusmn; SE) was `r mean_soil_moisture_T0` &plusmn; `r se_soil_moisture_T0`% based on a dry weight gravimetric measurement, while the average soil pH (&plusmn; SE) was `r mean_pH_T0` &plusmn; `r se_pH_T0`. Mean C:N ratio (&plusmn; SE) was `r mean_CN_T0` &plusmn; `r se_CN_T0`, while mean microbial enzymatic activity (&plusmn; SE) was `r mean_phenox_T0` &plusmn; `r se_phenox_T0` &mu;mol substrate per g soil per hour for phenol oxidase and `r mean_perox_T0` &plusmn; `r se_perox_T0` &mu;mol substrate per g soil per hour for peroxidase concentration. The mean total microfauna, mesofauna, and total fauna abundances (&plusmn; SE) were: `r mean_mico_abundance_T0` (&plusmn; `r se_micro_abundance_T0`), `r mean_meso_abundance_T0` (&plusmn; `r se_meso_abundance_T0`), and `r mean_total_abundance_T0` (&plusmn; `r se_total_abundance_T0`) individuals per m^2^, respectively.
<br>
<br>
...
Converting faunal abundance to biomass estimates, I calculated mean microfauna, mesofauna and total fauna biomass (&plusmn; SE) as `r mean_micro_biomass_T0` (&plusmn; `r se_micro_biomass_T0`), `r mean_meso_biomass_T0` (&plusmn; `r se_meso_bioamss_T0`), and `r mean_total_biomass_T0` (&plusmn; `r se_total_biomass_T0`) g of C per m^2^, respectively. Mean total consumptive flux (&plusmn; SE) based on the energetic model was `r mean_total_consumption_T0` &plusmn; `r se_total_consumption_T0` g of C per m^2^ per year, while the mean total C mineralization (&plusmn; SE) was `r mean_total_Cmin_T0` &plusmn; `r se_total_Cmin_T0` g of C per m^2^ per year, and the mean total N mineralization (&plusmn; SE) was `r mean_total_Nmin_T0` &plusmn; `r se_total_Nmin_T0` g of N per m^2^ per year. Mean values are presented in [Appendix D].
<br>
<br>
