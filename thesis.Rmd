...
WORK IN PROGRESS. LAST UPDATED `r Sys.Date()`
...
In the following thesis, the soil fauna abundance, biomass, and food web modelling following three-week perturbation (i.e., T~1~ data) was published in a Special Issue on soil food web (Pettit et al. 2023):
...
Mean soil moisture ([Figure 3.2]A) was significantly related to the induced soil moisture treatments and significantly greater in saturated vs. field-moist treatments (F~`r soil_moisture_aov_soil_moisture_df`,`r soil_moisture_aov_residuals_df`~ = `r soil_moisture_aov_soil_moisture_F`, p `r ifelse(soil_moisture_aov_soil_moisture_p < 0.001, "&lt; 0.001", paste0("= ", format(nsmall = 3)))`) and was significantly greater in 20 &deg;C vs. 12 &deg;C treatments (F~`r soil_moisture_aov_temp_df`,`r soil_moisture_aov_residuals_df`~ = `r soil_moisture_aov_temp_F`, p `r ifelse(soil_moisture_aov_temp_p < 0.001, "&lt; 0.001", paste0("= ", format(round(soil_moisture_aov_temp_p, 3), nsmall = 3)))`). Soil moisture was not affected by destructive sampling time (F~`r soil_moisture_aov_destructive_df`,`r soil_moisture_aov_residuals_df`~ = `r soil_moisture_aov_destructive_F`, p `r ifelse(soil_moisture_aov_destructive_p < 0.001, "&lt; 0.001", paste0("= ", format(round(soil_moisture_aov_destructive_p, 3), nsmall = 3)))`) nor block effects (F~`r soil_moisture_aov_block_effect_df`,`r soil_moisture_aov_residuals_df`~ = `r soil_moisture_aov_block_effect_F`, p `r ifelse(soil_moisture_aov_block_effect_p < 0.001, "&lt; 0.001", paste0("= ", format(round(soil_moisture_aov_block_effect_p, 3), nsmall = 3)))`). There were no significant interaction effects between soil moisture, temperature, and destructive sampling time ([Appendix E]). Mean soil moisture is presented in [Appendix D]. Estimates of effect size based on a linear model are given in [Appendix F].
<br>
<br>
...
Mean soil pH ([Figure 3.2]B) was not significantly influenced by soil moisture (F~`r pH_aov_soil_moisture_df`,`r pH_aov_residuals_df`~ = `r pH_aov_soil_moisture_F`, p `r ifelse(pH_aov_soil_moisture_p < 0.001, "&lt; 0.001", paste0("= ", format(round(pH_aov_soil_moisture_p, 3), nsmall = 3)))`) nor temperature (F~`r pH_aov_temp_df`,`r pH_aov_residuals_df`~ = `r pH_aov_temp_F`, p `r ifelse(pH_aov_temp_p < 0.001, "&lt; 0.001", paste0("= ", format(round(pH_aov_temp_p, 3), nsmall = 3)))`) treatments. Destructive sampling time exhibited a significant effect on pH (F~`r pH_aov_destructive_df`,`r pH_aov_residuals_df`~ = `r pH_aov_destructive_F`, p `r ifelse(pH_aov_destructive_p < 0.001, "&lt; 0.001", paste0("= ", format(round(pH_aov_destructive_p, 3), nsmall = 3)))`) and pH tended to be greater in mesocosms sampled at T~2~ vs. T~1~, but block effects did not explain differences in pH between treatments (F~`r pH_aov_block_effect_df`,`r pH_aov_residuals_df`~ = `r pH_aov_block_effect_F`, p `r ifelse(pH_aov_block_effect_p < 0.001, "&lt; 0.001", paste0("= ", format(round(pH_aov_block_effect_p, 3), nsmall = 3)))`). There were no significant interaction effects between soil moisture, temperature, and destructive sampling time ([Appendix E]). Mean soil pH is presented in [Appendix D]. Estimates of effect size based on a linear model are given in [Appendix F].
<br>
<br>
  
### Supplemental Table 3.7
*ANOVA results for log-transformed soil pH across experimental treatments temperature (12 &deg;C, 20 &deg;C), soil moisture (field-moist, saturated) and destructively sampling (T~1~: after three-weeks, T~2~: following three-week recovery period) treatments.*
```{r Soil Results 6, warning = F}
...
Soil C:N ([Figure 3.2]C) did not show any clear trends across treatments. Neither soil moisture (F~`r CN_aov_soil_moisture_df`,`r CN_aov_residuals_df`~ = `r CN_aov_soil_moisture_F`, p `r ifelse(CN_aov_soil_moisture_p < 0.001, "&lt; 0.001", paste0("= ", format(round(CN_aov_soil_moisture_p, 3), nsmall = 3)))`), temperature (F~`r CN_aov_temp_df`,`r CN_aov_residuals_df`~ = `r CN_aov_temp_F`, p `r ifelse(CN_aov_temp_p < 0.001, "&lt; 0.001", paste0("= ", format(round(CN_aov_temp_p, 3), nsmall = 3)))`), nor destructive sampling time (F~`r CN_aov_destructive_df`,`r CN_aov_residuals_df`~ = `r CN_aov_destructive_F`, p `r ifelse(CN_aov_destructive_p < 0.001, "&lt; 0.001", paste0("= ", format(round(CN_aov_destructive_p, 3), nsmall = 3)))`) had a significant effect on soil C:N ratio. There was no significant difference in C:N ratio between treatments corresponding to block effects (F~`r CN_aov_block_effect_df`,`r CN_aov_residuals_df`~ = `r CN_aov_block_effect_F`, p `r ifelse(CN_aov_block_effect_p < 0.001, "&lt; 0.001", paste0("= ", format(round(CN_aov_block_effect_p, 3), nsmall = 3)))`) and there were no significant interaction effects between soil moisture, temperature, and destructive sampling time ([Appendix E]). Mean C:N (&plusmn; SE) under each treatment is presented in [Appendix D]. Estimates of effect size based on a linear model are given in [Appendix F].
<br>
<br>
  
### Supplemental Table 3.8
*ANOVA results for soil C:N ratio across experimental treatments temperature (12 &deg;C, 20 &deg;C), soil moisture (field-moist, saturated) and destructively sampling (T~1~: after three-weeks, T~2~: following three-week recovery period) treatments.*
```{r Soil Results 8, warning = F}
