...
flux_data$tx <- interaction(flux_data$temp_tx, flux_data$moisture_tx) 
...
consumption_plot <- 
  ggplot(flux_data, aes(x=tx, y=total_consumption, 
                            fill = as.factor(destructive_time))) + 
  geom_boxplot(outlier.shape=NA) + #avoid plotting outliers twice
  geom_jitter(aes(pch = as.factor(destructive_time)), 
              position=position_jitter(width=.1, height=0), size = 3) +
...
Cmin_plot <- 
  ggplot(flux_data, aes(x=tx, y=total_Cmin, 
                            fill = as.factor(destructive_time))) + 
  geom_boxplot(outlier.shape=NA) + #avoid plotting outliers twice
  geom_jitter(aes(pch = as.factor(destructive_time)), 
              position=position_jitter(width=.1, height=0), size = 3) +
...
Nmin_plot <- 
  ggplot(flux_data, aes(x=tx, y=total_Nmin, 
                            fill = as.factor(destructive_time))) + 
  geom_boxplot(outlier.shape=NA) + #avoid plotting outliers twice
  geom_jitter(aes(pch = as.factor(destructive_time)), 
              position=position_jitter(width=.1, height=0), size = 3) +
...
flux_df1 <- as.data.frame(matrix(data = c(
  ...
  format(round(levene_test(total_consumption ~ 
                             (moisture_tx * temp_tx * destructive_time), 
              data = flux_data)$statistic, 3), nsmall = 3),
  format(round(levene_test(total_consumption ~ 
                             (moisture_tx * temp_tx * destructive_time), 
              data = flux_data)$p, 3), nsmall = 3),
...
```{r Flux Results 2}
consumption_aov <- Anova(consumption_lm2, type = 'III')
...
```{r Flux Results 3}
Cmin_aov <- Anova(Cmin_lm2, type = 'III')
...
```{r Flux Results 4}
Nmin_aov <- Anova(Nmin_lm2, type = 'III')
